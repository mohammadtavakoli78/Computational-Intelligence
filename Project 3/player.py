...
class Player(pygame.sprite.Sprite):
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        first_obstacle_x, first_obstacle_y, second_obstacle_x, second_obstacle_y, \
            third_obstacle_x, third_obstacle_y = 0, 0, 0, 0, 0, 0
        if len(obstacles) > 0:
            first_obstacle_x = player_x - obstacles[0]['x'] / screen_width
            first_obstacle_y = (player_y - obstacles[0]['y']/screen_height) * 3
            # if obstacles[0]['x'] == 177 or obstacles[0]['x'] == 410:
            #     first_obstacle_y = (obstacles[0]['y'] * 4) / (screen_height * 3)

        if len(obstacles) > 1:
            second_obstacle_x = player_x - obstacles[1]['x'] / screen_width
            second_obstacle_y = (player_y - obstacles[1]['y']/screen_height) * 2
            # if obstacles[1]['x'] == 177 or obstacles[1]['x'] == 410:
            #     second_obstacle_y = (obstacles[1]['y'] * 4) / (screen_height * 3)

        # if len(obstacles) > 2:
        #     third_obstacle_x = player_x - obstacles[2]['x'] / screen_width
        #     third_obstacle_y = (player_y - obstacles[2]['y']/screen_height) * 1
            # if obstacles[2]['x'] == 177 or obstacles[2]['x'] == 410:
            #     third_obstacle_y = (obstacles[2]['y'] * 4) / (screen_height * 3)

        input_vector = [player_x, player_y, first_obstacle_x, first_obstacle_y, second_obstacle_x, second_obstacle_y]
