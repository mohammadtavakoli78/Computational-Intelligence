...
def run_alogrithm(train_set, number_of_images, number_of_epochs, batch_size, weight_first_layer, weight_second_layer,
    ...
    for i in range(number_of_epochs):
        ...
        for counter in range(int(number_of_images/batch_size)):
            ...
            for image in train_set[counter * 10:(counter + 1) * 10]:
                ...
                for j in range(4):
                    for k in range(60):
                        grad_weight_third_layer[j, k] += a2[k][0] * sigmoid_deriv(z3[j][0]) * (
                                2 * a3[j][0] - 2 * output_label_local[j][0])
                        grad_bias_third_layer[j][0] += sigmoid_deriv(z3[j][0]) * (
                                2 * a3[j][0] - 2 * output_label_local[j][0])

                ga2 = np.zeros((60, 1))
                for k in range(60):
                    for j in range(4):
                        ga2[k, 0] += weight_third_layer[j, k] * sigmoid_deriv(z3[j, 0]) * (
                                2 * a3[j, 0] - 2 * output_label_local[j, 0])

                for j in range(60):
                    for k in range(150):
                        grad_weight_second_layer[j, k] += a1[k, 0] * sigmoid_deriv(z2[j, 0]) * (ga2[j][0])
                        grad_bias_second_layer[j][0] += sigmoid_deriv(z2[j, 0]) * (ga2[j][0])

                ga1 = np.zeros((150, 1))
                for k in range(150):
                    for j in range(60):
                        ga1[k, 0] += weight_second_layer[j, k] * sigmoid_deriv(z2[j, 0]) * ga2[j][0]

                for j in range(150):
                    for k in range(102):
                        grad_weight_first_layer[j, k] += input_image_local[k, 0] * sigmoid_deriv(z1[j, 0]) * ga1[j][0]
                        grad_bias_first_layer[j][0] += sigmoid_deriv(z1[j, 0]) * ga1[j][0]
    ...
    show_cost(3, number_of_epochs, costs)
...
if __name__ == "__main__":
    ...
    run_alogrithm(train_set=train_set, number_of_images=number_of_images, number_of_epochs=number_of_epochs, batch_size=batch_size,
