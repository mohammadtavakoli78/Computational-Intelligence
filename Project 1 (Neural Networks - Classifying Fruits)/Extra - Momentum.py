...
def run_alogrithm(train_set, number_of_images, number_of_epochs, batch_size, momentum, weight_first_layer, weight_second_layer,
                  weight_third_layer, bias_first_layer,
                  bias_second_layer, bias_third_layer):
    costs = []
    weight_first_layer_difference = 0
    weight_second_layer_difference = 0
    weight_third_layer_difference = 0
    ...
    for i in range(number_of_epochs):
        ...
        for counter in range(int(number_of_images / batch_size)):
            ...
            weight_first_layer_old = weight_first_layer
            weight_second_layer_old = weight_second_layer
            weight_third_layer_old = weight_third_layer

            weight_first_layer = weight_first_layer - (learning_rate * grad_weight_first_layer / batch_size) + (
                    momentum * weight_first_layer_difference)
            weight_second_layer = weight_second_layer - (learning_rate * grad_weight_second_layer / batch_size) + (
                        momentum * weight_second_layer_difference)
            weight_third_layer = weight_third_layer - (learning_rate * grad_weight_third_layer / batch_size) + (
                        momentum * weight_third_layer_difference)

            weight_first_layer_difference = abs(weight_first_layer - weight_first_layer_old)
            weight_second_layer_difference = abs(weight_second_layer - weight_second_layer_old)
            weight_third_layer_difference = abs(weight_third_layer - weight_third_layer_old)
    ...
    show_cost(4, number_of_epochs, costs)
...
if __name__ == "__main__":
    ...
    run_alogrithm(train_set=train_set, number_of_images=number_of_images, number_of_epochs=number_of_epochs,
                  batch_size=batch_size, momentum=0.3,
                  weight_first_layer=weight_first_layer, weight_second_layer=weight_second_layer,
